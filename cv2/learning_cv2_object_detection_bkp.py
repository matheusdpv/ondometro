...
for filename in filelist[200:201]:
	...
	cv2.drawContours(mask, [big_strawberry_contour], -1, 255, -1)
	...
	#easy function
	ellipse = cv2.fitEllipse(big_strawberry_contour)
	#add it
	...
	plt.imshow(image_with_ellipse, interpolation='nearest')


	#we're done, convert back to original color scheme
	result = cv2.cvtColor(image_with_ellipse, cv2.COLOR_RGB2BGR)
